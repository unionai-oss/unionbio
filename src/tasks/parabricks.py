...
def pb_fq2bam(reads: Reads, sites: Sites, ref: Reference) -> Alignment:
    ...
    """
    return Alignment(sample=reads.sample, aligner="pbrun_fq2bam")
...
def pb_deepvar(al: Alignment, ref: Reference) -> VCF:
    return VCF(sample=al.sample, caller="pbrun_deepvariant")
    # """
...
def pb_haplocall(al: Alignment, ref: Reference) -> VCF:
    ...
    """
    return VCF(sample=al.sample, caller="pbrun_haplocall")
    # ref.ref_dir.download()
    # al.alignment.download()
    # al.alignment_idx.download()
