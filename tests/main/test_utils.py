...
from unionbio.config import remote_sites_vcf, remote_sites_idx
...
def test_alignment_reformat(tmp_path):
    shutil.copy2(
        Path(test_assets["al_dir"]).joinpath(
            "recal/SRR812824-chr21_bowtie2_sorted_deduped_recal.sam"
        ),
        tmp_path,
    )
...
def test_filter_dir():
    out = [
        i
        for i in filter_dir(
            test_assets["vcf_dir"], include=["test-sample*"], exclude=["test-sample-2*"]
        )
    ]
    assert len(out) == 2
