...
def compare_dirs(expected: Path, actual: Path, mode: str) -> bool:
    assert mode in [
        ...
        "whole",
        "lines",
    ], "Please choose a valid comparison mode: exists, whole, lines"

    logger.debug(
        f"Comparing expected dir {expected} with test dir {actual} in mode {mode}"
    )
    ...
    for exp_file in exp_files:
        ...
        if mode == "exists":
            assert (
                ac_file.exists()
            ), f"Expected file {exp_file} does not exist in test directory"
        elif mode == "whole":
            assert filecmp.cmp(
                exp_file, ac_file
            ...
            assert comp_lines(
                exp_file, ac_file
            ), f"Expected file {exp_file} and test file {ac_file} failed line by line comparison"

    for exp_dir in exp_dirs:
        ac_dir = actual.joinpath(exp_dir.name)
        assert (
            ac_dir.exists()
        ), f"Expected subdirectory {ac_dir} does not exist in test directory"
